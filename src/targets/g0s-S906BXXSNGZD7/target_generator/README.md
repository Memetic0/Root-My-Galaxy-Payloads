...
> [!NOTE]
> This target generator is **only used to generate `target.h` for Samsung Galaxy S22+ (g0s)** devices because this repository's exploit is only ported for the S22+.
...
1. Create a folder in `src/targets/` named exactly after your firmware version (e.g. `src/targets/S906BXXSNGZD7`).
